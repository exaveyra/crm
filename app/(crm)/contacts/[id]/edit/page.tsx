...
export default function EditContactPage() {
  ...
  async function fetchContact() {
    ...
    if (data.contact) {
      ...
      setForm({
        ...
        notes: c.notes || "",
        hipaa_consent: c.hipaa_consent || false,
        estimated_monthly_value: c.estimated_monthly_value || 0,
      });
    }
    setLoading(false);
  }
  ...
  const isPractitioner = ["prescriber", "clinic_admin", "regenerative_md", "aesthetics_md"].includes(form.contact_type);
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        <Link href={`/contacts/${id}`} className="text-slate-400 hover:text-white transition">
          ← Back
        </Link>
        <div>
          <h2 className="text-2xl font-bold text-white">Edit Contact</h2>
          <p className="text-slate-400 text-sm mt-1">Update contact information</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-4">
          ...
          <div>
            <label className="block text-slate-400 text-sm mb-1.5">Est. Monthly Value ($)</label>
            <input
